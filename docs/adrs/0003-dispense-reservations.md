...
2. **Reservations are amount locks, not proposal-input locks.** The reservation row records the notes selected at the moment the reservation was recorded, but the enforcement contract is amount-based: the sum of active reservations for an account must stay at or below the wallet's shielded-spendable view. librustzcash output locks protect the shorter proposal-to-transaction-storage interval once Zally knows the concrete inputs. A dispense reservation begins earlier, can outlive a process, and may not yet have a recipient or proposal, so it reserves value independently. The note set is recorded for operator visibility and downstream observability.
...
- **Create and lock a proposal at reservation time.** Rejected: librustzcash now supports owner-scoped locks for concrete proposal inputs, and Zally uses them during transaction construction. A dispense reservation is intentionally earlier and longer-lived than a proposal, and the recipient or final amount may not yet be available. Building a speculative proposal would bind the reservation to an anchor, fee result, and input set that can become stale before dispense. Amount locking preserves the application-level guarantee while the upstream lock lifecycle protects the later construction interval.

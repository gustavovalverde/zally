...
Zally ships eight crates. The scan loop is a module inside `zally-wallet`, not a separate crate.

### The eight crates

| Crate | Role | Depends on |
|-------|------|------------|
| `zcash-payment-disclosure` | Portable ZIP-311 Draft1 and explicitly versioned Zally Ironwood disclosure codecs, production, and verification. The crate is intentionally free of Zally domain types so it can move upstream. | Zcash protocol and cryptography crates only |
...
| `zally-storage` | `WalletStorage` trait wrapping librustzcash's `WalletRead`/`WalletWrite`/`WalletCommitmentTrees`/`InputSource`. Default `Sqlite` implementation over `zcash_client_sqlite`. | `zally-core`, `zcash_client_backend`, `zcash_client_sqlite` |
...
| `zally-pczt` | PCZT roles for external signing and proving: `Creator`, `Prover`, `Signer`, `Combiner`, `Extractor`. Wraps `pczt` crate; adds Zally-typed error vocabulary and adapts retained PCZT material into portable payment-disclosure plans. | `zally-core`, `zally-keys`, `zcash-payment-disclosure`, `pczt`, `zcash_client_backend` |
...
zcash-payment-disclosure ──── zally-pczt ──── zally-wallet
```

`zally-core` has no internal dependencies. Every `zally-*` crate depends on it directly. The portable `zcash-payment-disclosure` crate deliberately does not. `zally-wallet` is the umbrella crate for operator integrations.
...
- Zally domain crate names: `zally-<noun>`. No `zally-<verb>`. A portable protocol crate uses its upstream-facing protocol name.
...
`zally-wallet` owns orchestration. `zally-chain`, `zally-storage`, `zally-keys`, and `zally-pczt` own focused boundaries. `zcash-payment-disclosure` owns the portable Draft1 protocol and the separately versioned Zally Ironwood extension. `zally-testkit` owns test fixtures and live-test gates.

...
| Field   | Value                                                                                                                                                                                                       |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status  | Accepted on 2026-05-26; revised 2026-05-27 (live-tip scan ceiling); revised 2026-06-03 (canonical scan-range orchestration replaces the hand-rolled resume loop)                                            |
...
2. **Subtree-root priming is mandatory and runs before planning.** `ChainSource::subtree_roots` carries each root's `completing_block_height`; the wallet records it through `put_subtree_roots` so a note is witness-anchored from its subtree root without scanning the whole subtree. This is the root-cause fix for the spendable-zero wedge: spendability no longer requires a full linear scan, and `suggest_scan_ranges` can plan subtree-aligned work.

3. **The scan ceiling is the live chain tip; the from_state invariant holds by construction.** `update_chain_tip` uses `chain.chain_tip()` (the live head), so the ZIP-315 anchor lands in scanned territory and tx-expiry math (`chain_tip + 1`, plus the expiry delta) stays valid. Because zinder serves an exact tree state at `range.start - 1`, `from_height == from_state.block_height() + 1` holds for every `scan_cached_blocks` call and the upstream assert can no longer fire. Scan ranges are clamped to the live tip: a suggested range can start above the tip when the birthday is ahead of the chain, which is not fetchable and is skipped.

4. **Reorg recovery is the library's Verify-range mechanism plus precise truncation.** `WalletStorage::scan_blocks` surfaces `StorageError::ChainReorgDetected { at_height }` (`Retryable`) on a continuity error. The loop fetches the chain state at `at_height - REORG_REWIND_BLOCKS` from the chain source and calls `WalletStorage::truncate_to_chain_state`, which lands the wallet at exactly that height; the next tick re-plans via `suggest_scan_ranges`. The `Verify`-priority range that `suggest_scan_ranges` returns first proactively re-validates the tip-adjacent blocks each cycle. The hand-rolled tip-regress detector, the `truncate_to_height`-based rewind (which snapped down to the nearest checkpoint and spiralled the scan backwards), and the rewind-margin arithmetic are deleted. `truncate_to_height` is removed from the storage trait. Divergences deeper than the librustzcash 100-block rewind cap (`COINBASE_MATURITY`) surface as `NotRetryable` and require an operator reset.

5. **`ChainSource::safe_chain_tip()` remains for the dispense reaper, not for sync.** It serves consumers that need a reorg-robust confirmation signal independent of scan progress: fauzec's dispense reaper uses it to decide when a broadcast has definitively expired, so a one-block reorg near the head cannot flip a `failed` row back to `submitted`. This is the same head/safe split Ethereum's Engine API exposes. Sync does not consult it.
...
- The `spendable_zat: 0` wedge is closed at its root: subtree roots witness-anchor notes, so `get_wallet_summary(ConfirmationsPolicy::default()).spendable_value()` reports the real balance once the tip range is scanned, without a full historical grind.
...
1. Confirm the chain source is healthy. Query `LatestBlock` and `LatestSafeBlock` from zinder; both should advance.
...
- **Scan only to `safe_chain_tip` (head minus reorg window).** Rejected previously and still rejected: any non-zero safe-tip lag re-creates the anchor-versus-ceiling gap; the canonical loop reaches the live tip via the `ChainTip` range and recovers reorgs precisely, so a safe-tip ceiling buys nothing for sync.
